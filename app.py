...
.kpi-card { background: linear-gradient(135deg, #1f2937 0%, #1a3a5c 100%); border: 1px solid #2196F3; border-radius: 10px; padding: 16px 20px; text-align: center; margin: 4px 0; }
...
.section-header { font-size: 16px; font-weight: 700; color: #64b5f6; border-left: 4px solid #2196F3; padding-left: 10px; margin: 20px 0 10px 0; }
...
DEFAULT_COUNTRIES = [c for c in ["Pakistan", "India", "China", "United States", "United Kingdom", "Bangladesh", "Australia", "Canada", "Germany", "France"] if c in all_countries]

with st.sidebar:
    st.markdown("## 🌍 Dashboard Filters")
    st.markdown("---")
    selected_indicators = st.multiselect("📊 Select Indicators", options=all_indicators, default=all_indicators[:3] if len(all_indicators) >= 3 else all_indicators)
    primary_indicator = st.selectbox("🎯 Primary Indicator", options=selected_indicators if selected_indicators else all_indicators)
    st.markdown("---")
    ...
    countries = st.multiselect("🌐 Countries", options=all_countries, default=DEFAULT_COUNTRIES)
...
st.markdown("<p style='color:#90caf9; margin-top:4px;'>World Bank DataBank | " + str(year_range[0]) + "–" + str(year_range[1]) + " | " + str(len(countries) if countries else len(all_countries)) + " countries | <b>" + str(len(df)) + "</b> filtered records</p>", unsafe_allow_html=True)
st.markdown("---")
...
def kpi_card(col, label, value, sub=""):
    col.markdown("<div class='kpi-card'><div class='kpi-label'>" + label + "</div><div class='kpi-value'>" + str(value) + "</div><div class='kpi-sub'>" + sub + "</div></div>", unsafe_allow_html=True)

kpi_card(k1, "Total Records", str(kpis['total_records']))
kpi_card(k2, "Countries", str(kpis['total_countries']))
kpi_card(k3, "Indicators", str(kpis['total_indicators']))
kpi_card(k4, "Year Span", kpis["year_span"])
kpi_card(k5, "Highest Country", kpis["max_country"])

st.markdown("<br>", unsafe_allow_html=True)

if len(df) > 0:
    st.markdown('<div class="section-header">📊 Distribution and Proportions</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
    ...
    st.dataframe(df[show_cols].head(500).reset_index(drop=True), use_container_width=True, height=300)
else:
    st.warning("No data found. Please adjust your filters.")

st.markdown("---")
st.markdown("<center><small style='color:#555'>World Development Indicators Dashboard - World Bank Data</small></center>", unsafe_allow_html=True)
