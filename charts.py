...
PALETTE = ["#2196F3", "#4CAF50", "#FF9800", "#E91E63", "#9C27B0", "#00BCD4", "#FF5722", "#8BC34A", "#FFC107", "#3F51B5"]
BG = "#0d1117"
FG = "#e6edf3"
GRID = "#21262d"
ACC = "#2196F3"
...
def chart_pie(df, indicator=None):
    data = df[df["Series Name"] == indicator] if indicator else df
    top = data.groupby("Country Name")["Value"].mean().nlargest(8)
    fig, ax = _fig(8, 6)
    wedges, texts, autotexts = ax.pie(top.values, labels=top.index, autopct="%1.1f%%", colors=PALETTE[:len(top)], startangle=140, wedgeprops=dict(edgecolor=BG, linewidth=1.5))
    for t in texts: t.set_color(FG); t.set_fontsize(9)
    for at in autotexts: at.set_color(FG); at.set_fontsize(8)
    ax.set_title("Top 8 Countries - " + str(indicator if indicator else "All"), color=FG, fontsize=12, fontweight="bold")
    _style(fig, ax)
    ax.grid(False)
    fig.tight_layout()
    return fig

def chart_histogram(df, indicator=None):
    ...
    ax.set_title("Value Distribution - " + str(indicator if indicator else "All"), color=FG, fontsize=12, fontweight="bold")
    _style(fig, ax)
    fig.tight_layout()
    return fig

def chart_line(df, indicator=None, top_n=8):
    data = df[df["Series Name"] == indicator] if indicator else df
    ...
    for i, country in enumerate(top_countries):
        ...
        ax.plot(sub["Year"], sub["Value"], marker="o", markersize=3, linewidth=2, label=country, color=PALETTE[i % len(PALETTE)])
    ax.set_xlabel("Year", color=FG)
    ax.set_ylabel("Value", color=FG)
    ax.set_title("Trends Over Time - " + str(indicator if indicator else "All"), color=FG, fontsize=12, fontweight="bold")
    ax.legend(fontsize=8, framealpha=0.2, labelcolor=FG, facecolor=BG)
    _style(fig, ax)
    fig.tight_layout()
    return fig

def chart_bar(df, indicator=None):
    data = df[df["Series Name"] == indicator] if indicator else df
    ...
    ax.set_title("Top 15 Countries - " + str(indicator if indicator else "All"), color=FG, fontsize=12, fontweight="bold")
    _style(fig, ax)
    fig.tight_layout()
    return fig
...
def chart_scatter(df):
    ...
    if len(indicators) >= 2:
        ...
        ax.scatter(merged.iloc[:, 0], merged.iloc[:, 1], color=ACC, alpha=0.7, s=50)
        ax.set_xlabel(str(ind1)[:40], color=FG, fontsize=9)
        ax.set_ylabel(str(ind2)[:40], color=FG, fontsize=9)
        ...
        ax.set_xlabel("Year", color=FG)
        ax.set_ylabel("Value", color=FG)
        ax.set_title("Year vs Value", color=FG, fontsize=12, fontweight="bold")
    _style(fig, ax)
    fig.tight_layout()
    return fig
...
def chart_box(df):
    ...
    fig, ax = _fig(9, 5)
    if bp_data:
        bp = ax.boxplot(bp_data, labels=short_labels, patch_artist=True, medianprops=dict(color="#FF9800", linewidth=2))
        for patch, color in zip(bp["boxes"], PALETTE):
            patch.set_facecolor(color)
            patch.set_alpha(0.8)
    ax.set_xlabel("Indicator", color=FG)
    ax.set_ylabel("Value", color=FG)
    ax.set_title("Box Plot by Indicator", color=FG, fontsize=12, fontweight="bold")
...
def chart_heatmap(df, top_n=12):
    ...
    data = df[df["Country Name"].isin(top_countries) & (df["Year"] >= 2000)]
    pivot = data.groupby(["Country Name", "Year"])["Value"].mean().unstack(fill_value=np.nan)
    fig, ax = _fig(11, 6)
    sns.heatmap(pivot, ax=ax, cmap="YlOrRd", linewidths=0.3, linecolor=BG, cbar_kws={"shrink": 0.8})
    ax.set_title("Heatmap: Top Countries x Year", color=FG, fontsize=12, fontweight="bold")
    ax.set_xlabel("Year", color=FG)
...
def chart_area(df, indicator=None, top_n=6):
    data = df[df["Series Name"] == indicator] if indicator else df
    top_countries = data.groupby("Country Name")["Value"].mean().nlargest(top_n).index
    area_data = data[data["Country Name"].isin(top_countries)].groupby(["Year", "Country Name"])["Value"].mean().unstack(fill_value=0).sort_index()
    fig, ax = _fig(10, 5)
    ax.stackplot(area_data.index, area_data.T.values, labels=area_data.columns, colors=PALETTE[:len(area_data.columns)], alpha=0.8)
    ax.set_xlabel("Year", color=FG)
    ax.set_ylabel("Value", color=FG)
    ax.set_title("Cumulative Trends - " + str(indicator if indicator else "All"), color=FG, fontsize=12, fontweight="bold")
...
def chart_countplot(df):
    ...
    short_labels = [lbl[:25] + "..." if len(lbl) > 25 else lbl for lbl in counts.index]
    fig, ax = _fig(9, 5)
...
def chart_violin(df):
    ...
    short_labels = [ind[:20] + "..." if len(ind) > 20 else ind for ind in indicators[:len(vdata)]]
    fig, ax = _fig(9, 5)
    if vdata:
    ...
    ax.set_xlabel("Indicator", color=FG)
    ax.set_ylabel("Value", color=FG)
    ax.set_title("Violin Plot by Indicator", color=FG, fontsize=12, fontweight="bold")
    _style(fig, ax)
    fig.tight_layout()
    return fig
